...
def map_word_emb(corpus,_map):
    ...
    return np.array(rtn)
def get_word_emb_mat(embs):
    _map=pkl.load(open('mapaoh.pkl','rb'))
    rtn=np.random.randn(n+2,500)*.1
    for w,ind in _map.items():
        if ind!=n+1 and w in embs:
            rtn[ind:]=embs[w].flatten()
        else:
            print w
    return rtn
...
class Caption_Generator():
    def __init__(self, dim_in, dim_embed, dim_hidden, batch_size, n_lstm_steps, n_words, init_b=None,from_image=False,n_input=None,n_lstm_input=None,n_z=None,init_w=None):
    ...
    def _initialize_weights(self):
        ...
        init_val=xavier_init(self.n_input, self.n_z)
        if self.init_w is not None:
            init_val=self.init_w.astype(np.float32)
        with tf.device('/cpu:0'):
            om=tf.Variable(init_val,name='out_mean',trainable=trainability)
...
def train(learning_rate=0.001, continue_training=False):
    ...
    # init_emb=get_word_emb_mat(pkl.load(open('training_data/D_cbow_pdw_8B.pkl','rb')))
    # np.save('oh_wv',init_emb)
    init_emb=np.load('oh_wv.npy','r')
    ...
    caption_generator = Caption_Generator(dim_in, dim_hidden, dim_embed, batch_size, maxlen+2, 50000, np.zeros(50000).astype(np.float32),n_input=n_input,n_lstm_input=n_lstm_input,n_z=n_z,init_w=init_emb)
    ...
    losses=[]
    # losses=pkl.load(open('losses/loss_w_frozen.pkl','rb'))
    ...
    for epoch in range(n_epochs):
        ...
        pkl.dump(losses,open('losses/loss_w.pkl','wb'))
